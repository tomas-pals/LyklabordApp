...
into the app's iCloud ubiquity container (`iCloud.is.palsson.lyklabord`,
...
Note the tilde-encoding: the container id `iCloud.is.palsson.lyklabord` becomes
...
Copies `Documents/sessions/` from the app container (`is.palsson.lyklabord`) into
`./sessions/`. Requires Xcode 15+ (`xcrun devicectl`), device paired &
unlocked, app installed. (Simulator: use
`xcrun simctl get_app_container booted is.palsson.lyklabord data` and copy
